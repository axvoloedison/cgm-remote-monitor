...
console.log('timeago:language', ctx.language);
...
console.log('timeago:checkNotifications', sbx.extendedSettings);
    ...
    function buildMessage(agoDisplay) {
      ...
      lines.unshift(translate('Last received:') + ' ' + [translate(agoDisplay.value), translate(agoDisplay.label)].join(' '));
      return lines.join('\n');
    }

    function sendAlarm (opts) {
      var agoDisplay = timeago.calcDisplay(lastSGVEntry, sbx.time);
console.log('timeago:sendAlarm', agoDisplay, translate('Last received:'));
...
console.log('timeago:checkStatus', status);
