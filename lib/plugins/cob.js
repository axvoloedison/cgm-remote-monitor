...
function init(ctx) {
  ...
  cob.fromDeviceStatus = function fromDeviceStatus(devicestatusEntry) {
    ...
    if (_.get(devicestatusEntry, 'openaps') !== undefined) {
      ...
      var lastCOB = null;
      var lastMoment = null;
      var lastCarbsReq = null;

//console.warn('COB plugin carbsReq');
      ...
      if (suggested && enacted) {
        ...
        if (enactedMoment.isAfter(suggestedMoment)) {
          ...
          lastCarbsReq = suggested.carbsReq;
        } else {
...
//console.warn('COB plugin carbsReq='+lastCarbsReq);

      if (lastCOB === null || !lastMoment) {
        return {};
      }

      if ( lastCarbsReq === null || lastCarbsReq === undefined ) {
        lastCarbsReq = '-0.01';
      }
...
//console.warn('COB plugin carbsReq='+lastCarbsReq);
  ...
  // console.log('cob.updateVisualisation l');
  // console.error('cob.updateVisualisation e');
  // console.warn('cob.updateVisualisation w');
  
    var prop = sbx.properties.cob;

// console.warn('COB prop.cob.carbsReq before prop === undefined || prop.cob === undefined');
    if (prop === undefined || prop.cob === undefined) { return; }

    var carbsReq = 0;

// console.warn('before COB prop.cob.carbsReq');

    if (prop.cob.carbsReq !== undefined ) {
      carbsReq = prop.cob.carbsReq;
      // console.warn('COB prop.cob.carbsReq set something');
    } else {
// console.warn('COB prop.cob.carbsReq IS NULL undefined');
    }
// console.warn('after COB prop.cob.carbsReq');
    ...
    var textCob = displayCob + 'g';

    if (carbsReq != 0) {
      textCob = textCob + ' Req ' + carbsReq + 'g'
    } else {
      textCob = textCob + ' No Req'      
    }
